...
- **D-02b (auth-tag packing, from research — highest crypto risk):** Web Crypto appends the 16-byte GCM auth tag to the ciphertext automatically, but `react-native-quick-crypto` (Node-style `createCipheriv`) returns the tag **separately** via `getAuthTag()`. Therefore `crypto.native.ts` must **concat-on-encrypt** (ciphertext + tag) and **split-on-decrypt** (last 16 bytes → `setAuthTag()` before `final()`) to reproduce the exact `[iv][ct+tag]` bytes. This concat/split lives in `crypto.native.ts` (only the native primitive needs it); the byte-layout constants/packing stay in the shared core. The cross-decrypt vector assertion (web-encrypted → native-decrypt and reverse) is mandatory — it is what catches an off-by-16 tag bug that would still round-trip native-to-native.
...
- **D-04:** The gate is **golden test vectors (byte-equality) + one manual device round-trip.** Shared golden vectors (one TS/JSON fixture: fixed key, fixed IV, fixed plaintext → expected exact packed-ciphertext bytes + SHA-256 hex + Base64URL) live in the shared package and are imported by every test suite so there is one source of expected bytes.
  - **Web:** a **Vitest** test asserts `crypto.web.ts` produces the exact vector bytes.
  - **Native (real primitive):** the throwaway harness screen runs the vectors against the **real** `crypto.native.ts` on-device and renders PASS/FAIL; a **Maestro** flow asserts the rendered result against an **EAS dev-client build** (see D-05).
  - **Cross-decrypt (mandatory):** web-encrypted vector decrypts to plaintext on native and vice-versa — this is what catches an off-by-16 auth-tag bug (D-02b).
...
- **D-05 (CORRECTED 2026-07-12 after research — supersedes the original "jest-expo runs the native vectors"):** `jest-expo` **cannot** execute the real `react-native-quick-crypto` JSI/Nitro binding — it mocks native modules and runs in Node/jsdom, so it can only test the **shared pure-TS core** (packing/splitting/Base64URL) as a fast pre-check, NOT the real native primitive. The **real native gate therefore runs on-device**: the harness screen executes the vectors against real `crypto.native.ts`, and **Maestro** (run locally via `maestro test`, or in CI via EAS Workflows) launches the EAS dev-client build and asserts the rendered PASS/FAIL. ROADMAP's success criterion ("automated interop test... must pass before uploader/downloader") mandates no specific runner, so Maestro satisfies the intent. `eas-cli` (via `npx eas-cli`) builds the dev-client binary Maestro drives.
...
### Claude's Discretion
- Final split of `packages/*` (one shared package vs `packages/crypto` + `packages/shared`), workspace/package naming (current root package is named `ffsend-web` — rename as sensible), and exact Maestro/jest-expo/EAS wiring are left to research/planning, provided the decisions above hold. See `05-RESEARCH.md` for verified versions (Expo SDK 57.0.4, quick-crypto 1.1.6) and the Maestro-on-device test pattern.
</decisions>
